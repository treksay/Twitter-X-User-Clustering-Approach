...
classes = ['TaylorSwift','WakandaForever','WorldCup','Ukraine']
sw = [ item.strip() for item in open("stopwords.txt").readlines() ]
...
def automatic_labeling(users, dataset, topics):
    labels = []
    for i in range(len(users)):
        tweets = getTweets(users[i], dataset)
...
def getMaxClassCounter(cluster, dataset):
    Classes = []
    Scores = []
    users = [x[1] for x in dataset]
    for i in range(len(cluster)):
        j = users.index(cluster[i])
        Class = dataset[j][4]
        if not(Class in Classes):
            Classes.append(Class)
            Scores.append(0)
            k = Classes.index(Class)
            Scores[k] = 1
        else:
            k = Classes.index(Class)
            Scores[k] += 1
...
def getEntropy(cluster, dataset):
    import math
    n = len(cluster)
    entropy = 0
    #Calculate Classes
    ...
    for i in classCount:
        if i > 0:
            entropy = entropy + ( (i/n) * math.log(i/n) )
    return -entropy

def combinations(cluster, label, data):
    n=0
    for i in range(len(cluster)):
        j = 0
        while(data[j][1]!=cluster[i]):
            j+=1
        if data[j][4] == label:
            n+=1
    return n*(n-1)/2
...
if __name__ == '__main__':
    ...
    for i in range(len(Pseudousers)):
        ...
        for j in range(len(Pseudousers[i][1])):         
            tw = tw+' '+ re.sub("#[A-Za-z0-9_]+", "", Pseudousers[i][1][j])
        dataTweets.append(tw)

    # LDA

    # Clean Original Seeds
    datasetCLEAN = dataTweets
    ...
    # Experimental Studies: LDA classifier

    # Calculate Purity
    MaxClasses = 0
    N = len(users)
    for i in range(len(FinalClusters)):
        maxClassScore = getMaxClassCounter(FinalClusters[i], dataset)
        MaxClasses += maxClassScore
    Purity = MaxClasses/N
    ...
    # Calculate Entropy
    for i in range(len(FinalClusters)):
        ClusterEntropy = getEntropy(FinalClusters[i], datasetCLEAN)
        print(f'Cluster {i} Entropy : {ClusterEntropy}')  
        Sum = Sum + (ClusterEntropy * len(FinalClusters[i]))
    Entropy = Sum / N
    ...
    for i in range(len(FinalClusters)):
        for j in range(len(classes)):
            TP = TP + combinations(FinalClusters[i], classes[j], dataset)
            y1=0
            y2=0
            x = count(classes[j], FinalClusters[i], dataset, 0) # 0 = Count the class
            for k in (range(i+1, len(FinalClusters))):
                y1 = y1 + count(classes[j], FinalClusters[k], dataset, 0)
                y2 = y2 + count(classes[j], FinalClusters[k], dataset, 1) # 1 = Count other than the class
            FN = FN + x*y1
            TN = TN + x*y2
        TPFP = TPFP + pairs(len(FinalClusters[i]))    
    FP = TPFP - TP
