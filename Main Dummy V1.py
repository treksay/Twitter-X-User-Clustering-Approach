...
classes = ['BlackAdam','WakandaForever','ThorLoveAndThunder','TheWayOfWater']
...
if __name__ == '__main__':
    ...
    for i in range(len(FinalClusters)):
        maxClassScore = getMaxClassCounter(FinalClusters[i], datasetCLEAN)
    ...
    for i in range(len(FinalClusters)):
        for j in range(len(classes)):
            TP = TP + combinations(FinalClusters[i], classes[j], datasetCLEAN)
            y1=0
            y2=0
            x = count(classes[j], FinalClusters[i], datasetCLEAN, 0) # 0 = Count the class
            for k in (range(i+1, len(FinalClusters))):
                y1 = y1 + count(classes[j], FinalClusters[k], datasetCLEAN, 0)
                y2 = y2 + count(classes[j], FinalClusters[k], datasetCLEAN, 1) # 1 = Count other than the class
            FN = FN + x*y1
            TN = TN + x*y2
